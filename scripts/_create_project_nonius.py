...
proj = req('POST', 'Project', body={"input": {
    "name": "Implantação Nonius — Magna Praia",
    "content": (
        "<p>Projeto de implantação da plataforma <strong>Nonius</strong> no hotel "
        "<strong>Carmel Magna Praia</strong>. A Nonius é uma solução de tecnologia para "
...
pid = proj['id']
print(f"OK  Project #{pid} criado: Implantação Nonius — Magna Praia")

# Vincular Jadson como membro do projeto (tipo 1 = membro, tipo 2 = gestor)
pu = req('POST', 'Project_User', body={"input": {
    "projects_id": pid,
    "users_id": JADSON,
    "type": 2,  # gestor
}}, st=st)
print(f"{'OK' if pu and 'id' in pu else 'WARN/skip'}  Jadson vinculado como gestor")

# Tarefas iniciais do projeto
tasks = [
    ("Levantamento de requisitos e escopo com a Nonius",   1, "2026-06-16 00:00:00"),
    ("Mapeamento de infraestrutura de rede — Magna Praia", 1, "2026-06-20 00:00:00"),
...
for name, status, plan_end in tasks:
    t = req('POST', 'ProjectTask', body={"input": {
        "projects_id": pid,
        "name": name,
        "status": status,  # 1 = Em planejamento
